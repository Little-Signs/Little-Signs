...
window.addEventListener(
  ...
  function () {
    ...
    set_heigth_sticky();
    sticky_init();
    jQuery(".portfolio_carousel").cws_flex_carousel(
      ".cws_portfolio",
      ".cws_portfolio_header"
    );
    jQuery(".ourteam_carousel").cws_flex_carousel(
      ".cws_ourteam",
      ".cws_ourteam_header"
    );
...
function isotope_init() {
  if (jQuery(".isotope").length) {
    ...
    if (
      jQuery(".cws_portfolio_filter_container .cws_portfolio_filter").length
    ) {
      var selector = document.querySelector(
        "select.cws_portfolio_filter"
      ).value;
      $container.isotope({ filter: selector });
    }

    jQuery(".cws_portfolio_filter_container").on(
      "change",
      "select.cws_portfolio_filter",
      function () {
...
jQuery(document).ready(function () {
  ...
  jQuery(".cws_ce_content.ce_toggle").cws_toggle(
    "accordion_section",
    "accordion_title",
    "accordion_content"
  );
...
function star_rating() {
  ...
  jQuery(".woocommerce .stars").on("mouseover", function () {
    if (!stars_active) {
      jQuery(this)
        .find("span:not(.stars-active)")
        .append(
          "<span class='stars-active' data-set='no'>&#xf005;&#xf005;&#xf005;&#xf005;&#xf005;</span>"
        );
      stars_active = true;
    }
  });
  jQuery(".woocommerce .stars").on("mousemove", function (e) {
    ...
    jQuery(".woocommerce .stars span a").css({
      "line-height": String((width + 1) / 5) + "px",
      width: String(width / 5) + "px",
    });
    jQuery(".woocommerce .stars span .stars-active").css("margin-top", "0px");
    jQuery(this)
      .find(".stars-active")
      .css("width", String(persent) + "%");
  ...
  jQuery(".woocommerce .stars").on("click", function (e) {
    ...
    rating.val(
      jQuery(
        jQuery(this).find("span a[class*='star-']")[
          Math.ceil(persent.toFixed(2) / 20) - 1
        ]
      ).text()
    );
  });
...
function woocommerce_price_slider() {
  ...
  jQuery(".price_slider").slider({
    ...
    create: function (event, ui) {
      ...
      jQuery("body").trigger("price_slider_create", [
        current_min_price,
        current_max_price,
      ]);
    },
    slide: function (event, ui) {
      ...
      jQuery("body").trigger("price_slider_slide", [
        ui.values[0],
        ui.values[1],
      ]);
    },
    change: function (event, ui) {
      jQuery("body").trigger("price_slider_change", [
        ui.values[0],
        ui.values[1],
      ]);
    },
...
function checkout_open_field() {
  jQuery(".woocommerce-info .showlogin, .woocommerce-info .showcoupon").on(
    "click",
    function () {
  ...
  jQuery(".payment_method_cheque input, .payment_method_paypal input").on(
    "click",
    function () {
      ...
      if (jQuery(x).hasClass("active")) {
        ...
        jQuery(
          ".payment_method_cheque .payment_box, .payment_method_paypal .payment_box"
        )
          .removeClass("active")
          .slideUp();
        jQuery(x).addClass("active").slideDown(300);
...
function scroll_top_controller() {
  ...
  if (
    scroll_pos < 1 &&
    window.scroll_top.el.hasClass(window.scroll_top.anim_in_class)
  ) {
    window.scroll_top.el.css({
      "pointer-events": "none",
    });
  ...
  jQuery.fn.cws_icon_popup = function () {
    jQuery(this).each(function () {
      ...
      function pressHandler(event) {
        ...
        TweenMax.to(
          jQuery(this)
            .children(".link-toggle-button")
            .children(".link-toggle-icon"),
          0.4,
          {
            rotation: on ? 45 : 0,
...
function cws_top_panel_slider() {
  jQuery(".site_top_panel.slider .site_top_panel_toggle").on(
    "click",
    function () {
      var el = jQuery(this);
...
function cws_top_panel_search() {
  jQuery(".site_top_panel .search_icon").on("click", function () {
    var el = jQuery(this);
    el.parents(".site_top_panel")
      .find(".row_text_search .search-field")
      .focus();
...
function cws_patern_width() {
  if (jQuery(".page_content>.pattern").length) {
    if (jQuery(".page_content").children().hasClass("container")) {
      jQuery(".page_content>.pattern").width(
        (jQuery("body").width() - jQuery(".page_content>.container").width()) /
          2 -
          20
      );
    } else {
      jQuery(".page_content>.pattern").width(
        (jQuery("body").width() -
          jQuery(".page_content>main .grid_row").width()) /
          2 -
          20
      );
    }
...
function cws_touch_events_fix() {
  if (is_mobile_device()) {
    jQuery(".page_content").on(
      "hover",
      ".hover-effect, .pic .links, title_part>a",
      function () {}
    );
    jQuery(".site_header").on(
      "hover",
      ".mobile_nav .button_open, .mobile_nav .menu-item > a",
      function (e) {
        e.preventDefault();
...
function cws_page_focus() {
  document
    ...
    .setAttribute("data-focus-chek", "focused");
...
function wp_standard_processing() {
  ...
  jQuery(
    ".gallery-icon a[href*='.jpg'], .gallery-icon a[href*='.jpeg'], .gallery-icon a[href*='.png'], .gallery-icon a[href*='.gif'], .cws_img_frame[href*='.jpg'], .cws_img_frame[href*='.jpeg'], .cws_img_frame[href*='.png'], .cws_img_frame[href*='.gif']"
  ).fancybox();
}
...
function sticky_vars() {
  window.sticky_menu = {
    ...
    menu_item_height: jQuery(".site_header .header_nav_part")
      .find(".main-menu>.menu-item")
      .eq(0)
      .outerHeight(),
    logo_indent: 12,
    header_content_part_width: parseInt(
      jQuery(".site_header .container").eq(0).width(),
      10
    ),
    animation_speed: 300,
  };
  if (window.sticky_menu.logo_position == "logo-center") {
    if (window.sticky_menu.site_header.find(".header_logo_part").length) {
      window.sticky_menu.menu_offset =
        jQuery(".header_nav_part").offset().top -
        window.sticky_menu.logo_indent;
    } else {
      window.sticky_menu.menu_offset = jQuery(".header_nav_part").offset().top;
    }
  } else {
    window.sticky_menu.menu_offset = window.sticky_menu.site_header
      .find(".header_box")
      .offset().top;
    if (window.header_after_slider) {
      window.sticky_menu.menu_offset =
        window.sticky_menu.menu_offset +
        window.sticky_menu.site_header.find(".header_box").outerHeight();
    }
  }
  if (window.sticky_menu.site_header.find(".header_logo_part").length) {
    window.sticky_menu.logo_top_init_margin =
      window.sticky_menu.logo_position == "logo-center"
        ? parseInt(
            jQuery(".site_header .header_logo_part").css("padding-top"),
            10
          )
        : parseInt(jQuery(".site_header .header_box").css("padding-top"), 10);
    window.sticky_menu.logo_bottom_init_margin =
      window.sticky_menu.logo_position == "logo-center"
        ? parseInt(
            jQuery(".site_header .header_logo_part").css("padding-bottom"),
            10
          )
        : parseInt(
            jQuery(".site_header .header_box").css("padding-bottom"),
            10
          );
  } else {
    window.sticky_menu.logo_top_init_margin =
      window.sticky_menu.logo_position == "logo-center"
        ? parseInt(
            jQuery(".site_header .header_nav_part").css("padding-top"),
            10
          )
        : parseInt(jQuery(".site_header .header_box").css("padding-top"), 10);
    window.sticky_menu.logo_bottom_init_margin =
      window.sticky_menu.logo_position == "logo-center"
        ? parseInt(
            jQuery(".site_header .header_nav_part").css("padding-bottom"),
            10
          )
        : parseInt(
            jQuery(".site_header .header_box").css("padding-bottom"),
...
function sticky(e) {
  ...
  var should_be_sticky =
    jQuery(document).scrollTop() > window.sticky_menu.menu_offset;
  var header_bg_sections = jQuery(
    ".header_bg_img, .cws_parallax_scene_container"
  );
  ...
  if (e_type == "resize") {
    ...
    if (!f_is_mobile) {
      if (
        should_be_sticky &&
        !is_set &&
        (!is_mobile() || mobile_menu_width_check())
      ) {
        set_sticky(true);
      } else if (
        (!should_be_sticky && is_set) ||
        (is_mobile() && !mobile_menu_width_check())
      ) {
        reset_sticky(true);
      }
    ...
    if (should_be_sticky && !is_set && mobile_menu_width_check()) {
      if (header_bg_sections.length) {
        cws_wait_for_header_bg_height_assigned(function () {
          /* it may require in another conditions too */ set_sticky();
        });
      } else {
        set_sticky();
      }
...
function set_sticky(animated) {
  ...
  var width = Boolean(jQuery(".page_boxed").length)
    ? jQuery(".page_boxed").outerWidth() + "px"
    : "100%";
  var left = Boolean(jQuery(".page_boxed").length)
    ? jQuery(".page_boxed").offset().left + "px"
    : "0";
  ...
  var logo_sticky_adjustments = "";
  if (
    window.sticky_menu.menu_item_height - 24 <
  ...
  if (
    window.sticky_menu.site_header
      .find(".header_logo_part")
      .hasClass("with_border")
  ) {
    if (
      window.sticky_menu.menu_item_height - 24 <
      window.sticky_menu.logo_init_height
  ...
  if (
    window.sticky_menu.logo_position == "logo-in" ||
    window.sticky_menu.site_header.hasClass("custom_sticky_logo")
  ) {
  ...
  if (window.sticky_menu.logo_position != "logo-center") {
    ...
    if (window.header_after_slider) {
      window.sticky_menu.site_header
        .find(".header_box")
        .css(tb_spacing_adjustments);
      window.sticky_menu.site_header
        .find(".logo>img:not(.logo_sticky)")
        .css(logo_sticky_adjustments);
    } else {
      if (animated) {
        window.sticky_menu.site_header
          .find(".header_box")
          .stop()
          .animate(tb_spacing_adjustments, window.sticky_menu.animation_speed);
        window.sticky_menu.site_header
          .find(".logo>img:not(.logo_sticky)")
          .stop()
          .animate(logo_sticky_adjustments, window.sticky_menu.animation_speed);
      } else {
        window.sticky_menu.site_header
          .find(".header_box")
          .css(tb_spacing_adjustments);
    ...
    window.sticky_menu.menu_container
      .find(".main-nav-container")
      .css({
        width: window.sticky_menu.header_content_part_width + "px",
        "margin-left": "auto",
        "margin-right": "auto",
      });
...
function reset_sticky(animated) {
  animated = animated != undefined ? animated : false;
  var sticky_obj_styles =
    window.sticky_menu.site_header.find(".header_box")[0].style;
  var logo_sticky_adjustments = "";
  if (
    window.sticky_menu.menu_item_height - 24 <
    window.sticky_menu.logo_init_height
  ...
  if (window.sticky_menu.logo_position != "logo-center") {
    ...
    if (!window.header_after_slider) {
      if (animated) {
        ...
        window.sticky_menu.site_header
          .find(".header_box")
          .css(tb_spacing_adjustments, window.sticky_menu.animation_speed);
        window.sticky_menu.site_header
          .find(".logo>img:not(.logo_sticky)")
          .css(logo_sticky_adjustments, window.sticky_menu.animation_speed);
      }
    }
  } else {
...
function mobile_menu_width_check() {
  if (jQuery(".site_header.logo-in-menu .container").length) {
    return (
      jQuery(".site_header .container").width() >
      parseInt(jQuery(".site_header .main-menu").attr("data-menu-width"), 10)
    );
  } else {
    return (
      jQuery(".site_header .container").width() >
      parseInt(jQuery(".site_header .main-menu").attr("data-menu-width"), 10) +
        jQuery(".site_header:not(.logo-center) .header_logo_part").width()
    );
  }
}

function logo_in_menu_replece() {
  var html_text = jQuery(
    ".site_header.logo-in-menu .header_logo_part"
  ).clone()[0];
  var i = parseInt(
    jQuery(".site_header.logo-in-menu").attr("data-menu-after"),
    10
  );
  var z;
  ...
  if (i <= 0) {
    ...
    jQuery(".site_header.logo-in-menu").length
      ? jQuery(z).after(html_text)
      : "";
  }
}
...
function check_menu_width() {
  var allWidth = 0;
  jQuery(".site_header .main-menu")
    .children()
    .each(function () {
      allWidth = allWidth + jQuery(this).outerWidth(true);
    });
  jQuery(".site_header.mobile_nav .main-menu").length
    ? ""
    : jQuery(".site_header .main-menu").attr("data-menu-width", allWidth);
}
...
function mobile_nav_switcher_init() {
  ...
  jQuery(document).on(
    "click",
    ".header_nav_part.mobile_nav .mobile_menu_header .mobile_menu_switcher",
    function () {
...
function mobile_nav_handlers_init() {
  jQuery(".header_nav_part.mobile_nav .button_open").on("click", function (e) {
    ...
    var left_a = jQuery(
      ".header_nav_part.mobile_nav .main-nav-container"
    ).hasClass("a-left");
    var right_a = jQuery(
      ".header_nav_part.mobile_nav .main-nav-container"
    ).hasClass("a-right");
...
function get_current_nav_level() {
  var r =
    window.mobile_nav.level < 2
      ? jQuery(".header_nav_part .main-menu")
      : jQuery(".main-nav-container .sub-menu");
  r.find(".sub-menu").remove();
  return r;
}

function get_next_nav_level(next_id) {
  var r = window.mobile_nav.nav_obj
    .find("#" + next_id)
    .children(".sub-menu")
    .first()
    .clone();
...
function mobile_menu_controller() {
  ...
  if (
    (is_mobile() && !window.mobile_nav.is_mobile_menu) ||
    !mobile_menu_width_check()
  ) {
    set_mobile_menu();
  } else if (
    !is_mobile() &&
    window.mobile_nav.is_mobile_menu &&
    mobile_menu_width_check()
  ) {
    reset_mobile_menu();
  } else {
...
function gallery_post_carousel_init() {
  jQuery(".gallery_post_carousel").each(function () {
    ...
    if (owl.attr("data-nav-init") == "true") {
      ...
      jQuery(this)
        .parent()
        .children(".carousel_nav.next")
        .on("click", function () {
          owl.trigger("owl.next");
        });
      jQuery(this)
        .parent()
        .children(".carousel_nav.prev")
        .on("click", function () {
...
function cws_portfolio_single_carousel_init() {
  jQuery(".cws_portfolio.single.related").each(function () {
    ...
    jQuery(".prev,.next", carousel_nav).on("click", function () {
      ...
      if (typeof next_ind != "number" || typeof all[next_ind] == undefined)
        return;
      next = all[next_ind];
      jQuery.post(
        ajaxurl,
...
jQuery.fn.cws_flex_carousel_controller = function (parent_sel, header_sel) {
  ...
  var show_hide_el_display_prop = window.getComputedStyle(
    show_hide_el[0]
  ).display;
    ...
    jQuery(this).each(function () {
      ...
      tabs.each(function () {
        jQuery(this).on("click", function () {
          var active_ind = jQuery(this)
            .siblings(".active")
            .eq(0)
            .attr("tabindex");
          jQuery(this).addClass("active").siblings().removeClass("active");
          var item = tab_items_container.find(
            "[tabindex='" + this.tabIndex + "']"
          );
          item
            .siblings("[tabindex='" + active_ind + "']")
            .eq(0)
            .fadeToggle("150", "swing", function () {
              item.fadeToggle("150");
            });
        });
      });
...
function cws_toggle_init() {
  jQuery.fn.cws_toggle = function (
    item_class,
    opener_class,
    toggle_section_class
  ) {
    var i = 0;
    jQuery(this).each(function () {
      i++;
    ...
    if (alt) {
      ...
      if (jQuery(icon_wrapp).parent("a").length) {
        jQuery(icon_wrapp).on("mouseenter", function () {
          jQuery(this)
            .find(".cws_fa")
            .css({ "background-color": font_color, color: bg_color });
        });
        jQuery(icon_wrapp).on("mouseleave", function () {
          jQuery(this)
            .find(".cws_fa")
            .css({ "background-color": bg_color, color: font_color });
        });
      }
    } else {
      jQuery(this).css({ "background-color": bg_color, color: font_color });
      ...
      if (jQuery(icon_wrapp).parent("a").length) {
        jQuery(icon_wrapp).on("mouseenter", function () {
          jQuery(this)
            .find(".cws_fa")
            .css({ "background-color": border_color, color: font_color });
...
function cws_button_colors_attach(
  alternative,
  el,
  style1,
  style2,
  ...
  jQuery(".contact-form").each(function () {
    jQuery(this).validate({
      ...
      submitHandler: function (form, event) {
        ...
        submit_handler(
          $form,
          jQuery(form).parent().children(".email_server_responce")
        );
      },
    });
  });
    ...
    jQuery.ajax({
	  ...
	  success: function (data) {
		$wrapper
          ...
          .append(
            '<div class="cws_msg_box success-box clearfix"><div class="icon_section"><i class="fa fa-thumbs-up"></i></div><div class="content_section"><div class="msg_box_title">Success!</div><div class="msg_box_text">Your message was successfully delivered.</div></div></div>'
          );
	  ...
	  error: function (jqXHR, textStatus, errorThrown) {
		$wrapper
          .addClass("cws_msg_box error-box clearfix")
          .append(
            "<div class='icon_section'><i class='fa fa-exclamation'></i></div><div class='content_section'><div class='msg_box_title'>Server fail!</div><div class='msg_box_text'><p> Please try again later!</p></div></div>"
          );
        $wrapper.delay(5000).hide(500, function () {
          jQuery(this)
            .removeClass("cws_msg_box error-box clearfix")
            .text("")
            .fadeIn(500);
          $wrapper.css("display", "none");
        });
...
/* Wrap this */
/**/
jQuery.fn.WrapThis = function (arg1, arg2) {
  /*=Takes 2 arguments, arg1 is how many elements to wrap together, arg2 is the element to wrap*/
    ...
    jQuery(this)
      ...
      .each(function (idx, item) {
        ...
        if (newNum % arg1 !== 0 && newNum <= lastArray) {
          ...
          //=If newNum is greater than the lastArray setting then start new array of elements
          arr.push(item);
        } else {
          //=If newNum is greater than the length of all the elements then wrap the remainder of elements in a column
          arr.push(item);
          var column = jQuery(this).pushStack(arr);
